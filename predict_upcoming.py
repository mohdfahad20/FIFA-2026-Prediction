...
from score_model.predict_score import predict_score, _load_params, most_likely_score_for_outcome
...
# ---------------------------------------------------------------------------
# Shared match_id helper — MUST match matches table format
# ---------------------------------------------------------------------------
...
def predict_one(home_db: str, away_db: str) -> dict:
    ...
    return {
        ...
        "pred_scoreline" : most_likely_score_for_outcome(ps["top_scorelines"], best[0]),
        "pred_winner"    : best[0],
    }
...
# ---------------------------------------------------------------------------
# Fill actuals — JOIN predictions with matches table for completed games
# ---------------------------------------------------------------------------
def fill_actuals(conn: sqlite3.Connection) -> int:
    """
    For any prediction row where actual_result IS NULL,
    check if the match now exists in the matches table (scraped).
    Uses make_match_id logic — both tables use same format.
    """
    rows = conn.execute("""
    ...
    for match_id, pred_winner, home_score, away_score, result in rows:
        # result in matches table: "win"=home won, "draw", "loss"=away won
        actual_side = (
            "home" if result == "win"
...
def main():
    ...
    try:
        ensure_table(conn)
        fill_actuals(conn)          # fill yesterday's actuals first
