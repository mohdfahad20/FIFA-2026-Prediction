...
with tab3:
    ...
    # Defensive: handle both {"tomorrow":[], "yesterday":[]} and flat list
    ...
    if not tomorrow_rows:
        ...
        cols = st.columns(2)
        for i, r in enumerate(tomorrow_rows):
            # Use .get() with safe fallbacks throughout
            hw       = r.get("pred_home_win", 0.0)
            dr       = r.get("pred_draw", 0.0)
            aw       = r.get("pred_away_win", 0.0)
            home_team = r.get("home_team", "—")
            away_team = r.get("away_team", "—")
            stage    = r.get("stage", "Group Stage")
            date_str = r.get("date", "")
            home_xg  = r.get("pred_home_xg", "—")
            away_xg  = r.get("pred_away_xg", "—")

            fav_prob = max(hw, dr, aw)
            fav = (home_team if hw == fav_prob
                   else away_team if aw == fav_prob
                   else "Draw")

            top3_raw = r.get("pred_top_scorelines")
            pred_scoreline = r.get("pred_scoreline", "N/A")
            try:
                top3 = json.loads(top3_raw) if top3_raw else [{"score": pred_scoreline, "probability": 1.0}]
            except (json.JSONDecodeError, TypeError):
                top3 = [{"score": pred_scoreline, "probability": 1.0}]

            top3_html = "".join(
                f'<span style="display:inline-block;background:#1e1e2e;border:1px solid #2a2a3e;'
                f'border-radius:6px;padding:0.25rem 0.65rem;margin-right:0.4rem;margin-bottom:0.3rem;font-size:0.8rem;">'
                f'<strong style="color:#f0f0f0;">{s.get("score","?")}</strong> '
                f'<span style="color:#00d26a;font-weight:600;">{s.get("probability",0):.0%}</span></span>'
                for s in top3
            )

            with cols[i % 2]:
                st.markdown(f"""
                <div style="background:#13131a;border:1px solid #1e1e2e;border-top:3px solid #00d26a;
                            border-radius:10px;padding:1.2rem;margin-bottom:1rem;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.75rem;">
                        <span style="font-size:0.7rem;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">{stage}</span>
                        <span style="font-size:0.7rem;color:#6b7280;">{date_str}</span>
                    </div>
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;">
                        <div style="text-align:center;flex:1;">
                            <div style="font-size:0.95rem;font-weight:700;color:#f0f0f0;">{home_team}</div>
                            <div style="font-size:1.4rem;font-weight:700;color:#00d26a;">{hw:.0%}</div>
                            <div style="font-size:0.7rem;color:#6b7280;">xG {home_xg}</div>
                        </div>
                        <div style="text-align:center;padding:0 0.5rem;">
                            <div style="font-size:0.8rem;font-weight:700;color:#6b7280;">VS</div>
                            <div style="font-size:0.75rem;font-weight:600;color:#f59e0b;margin-top:0.2rem;">Draw {dr:.0%}</div>
                        </div>
                        <div style="text-align:center;flex:1;">
                            <div style="font-size:0.95rem;font-weight:700;color:#f0f0f0;">{away_team}</div>
                            <div style="font-size:1.4rem;font-weight:700;color:#ef4444;">{aw:.0%}</div>
                            <div style="font-size:0.7rem;color:#6b7280;">xG {away_xg}</div>
                        </div>
                    </div>
                    <div style="background:#0d0d14;border-radius:6px;padding:0.6rem 0.75rem;">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.4rem;">
                            <span style="font-size:0.72rem;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;">🎯 Likely Scorelines</span>
                            <span style="font-size:0.75rem;font-weight:700;color:#00d26a;">Tip: {fav} ({fav_prob:.0%})</span>
                        </div>
                        <div>{top3_html}</div>
                    </div>
                </div>
    ...
    if not yesterday_rows:
        ...
        for i, r in enumerate(yesterday_rows):
            ...
            has_actual   = actual_home is not None
            ...
            pred_label = (home_team  if pred_winner == "home"
                          else away_team if pred_winner == "away"
                          else "Draw")

            actual_score_html = (
                f'<div style="font-size:0.9rem;font-weight:700;color:#f0f0f0;">'
                f'{actual_home} – {actual_away}</div>'
                f'<div style="font-size:0.75rem;color:#6b7280;">'
                f'{actual_result.capitalize()}</div>'
                if has_actual else
            ...
            with cols2[i % 2]:
                st.markdown(f"""
                <div style="background:#13131a;border:1px solid #1e1e2e;border-top:3px solid {border_color};
                            border-radius:10px;padding:1.2rem;margin-bottom:1rem;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.6rem;">
                        <span style="font-size:0.7rem;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">{stage}</span>
                        <span style="background:{badge_bg};color:{badge_color};font-size:0.7rem;
                    ...
                    <div style="font-size:1rem;font-weight:700;color:#f0f0f0;text-align:center;margin-bottom:0.75rem;">
                        {home_team} vs {away_team}
                    </div>
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;">
                        <div style="background:#1e1e2e;border-radius:8px;padding:0.6rem;text-align:center;">
                            <div style="font-size:0.65rem;font-weight:600;color:#6b7280;text-transform:uppercase;margin-bottom:0.3rem;">🔮 Predicted</div>
                            <div style="font-size:0.9rem;font-weight:700;color:#f0f0f0;">{pred_label}</div>
                            <div style="font-size:0.75rem;color:#6b7280;">Score: {pred_scoreline}</div>
                            <div style="font-size:0.7rem;color:#6b7280;">xG {home_xg} – {away_xg}</div>
                        </div>
                        <div style="background:{actual_bg};border-radius:8px;padding:0.6rem;text-align:center;">
                            <div style="font-size:0.65rem;font-weight:600;color:#6b7280;text-transform:uppercase;margin-bottom:0.3rem;">⚽ Actual</div>
                            {actual_score_html}
                        </div>
                    </div>
                </div>
...
with tab4:
    ...
    if st.button("Predict", key="btn_predict"):
        ...
        if result:
            ...
            with col_a:
                st.markdown("**Combined (ML + Poisson)**")
                prob_bar(f"{home} Win",  c.get("win", 0),  "#00d26a")
                prob_bar("Draw",          c.get("draw", 0), "#f59e0b")
                prob_bar(f"{away} Win",  c.get("loss", 0), "#ef4444")
            with col_b:
                st.markdown("**ML Model only**")
                prob_bar(f"{home} Win",  ml.get("win", 0),  "#00d26a")
                prob_bar("Draw",          ml.get("draw", 0), "#f59e0b")
                prob_bar(f"{away} Win",  ml.get("loss", 0), "#ef4444")
            st.markdown(f"""
            <div class="stat-card" style="margin-top:1rem">
                <div class="label">FIFA Rankings</div>
                <div style="font-size:1rem;margin-top:0.3rem">
                    {home} <strong>#{result.get('home_rank', '—')}</strong>
                    &nbsp;·&nbsp;
                    {away} <strong>#{result.get('away_rank', '—')}</strong>
                </div>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.error("Could not reach the backend. Please try again in a moment.")
...
with tab5:
    ...
    if st.button("Predict Score", key="btn_score"):
        ...
        if result:
            ...
            col_a,col_b,col_c = st.columns(3)
            with col_a:
                st.markdown(f'<div class="stat-card"><div class="label">{s_home} xG</div><div class="value">{xg.get("home","—")}</div></div>', unsafe_allow_html=True)
            with col_b:
                st.markdown(f'<div class="stat-card"><div class="label">Most Likely Score</div><div class="value">{result.get("most_likely_score","—")}</div></div>', unsafe_allow_html=True)
            with col_c:
                st.markdown(f'<div class="stat-card"><div class="label">{s_away} xG</div><div class="value">{xg.get("away","—")}</div></div>', unsafe_allow_html=True)
            st.markdown("---")
            scores_df = pd.DataFrame(result.get("top_scorelines", []))
            if not scores_df.empty:
                ...
                st.plotly_chart(fig, use_container_width=True)
            st.markdown("**Outcome probabilities (Poisson)**")
            prob_bar(f"{s_home} Win", result.get("win_prob", 0),  "#00d26a")
            prob_bar("Draw",           result.get("draw_prob", 0), "#f59e0b")
            prob_bar(f"{s_away} Win", result.get("loss_prob", 0), "#ef4444")
        else:
            st.error("Could not reach the backend. Please try again in a moment.")
...
Knockout: extra time → penalties (50/50) · Real completed matches locked in nightly
    ...
    if teams_data:
        ...
        st.info("Team data unavailable — backend may still be waking up.")